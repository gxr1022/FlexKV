...
| 组件 | 版本/位置 |
|---|---|
| venv | `/raid/xrg/sglang/.venv-flexkv`（Python 3.12，torch 2.13.0+cu130） |
| sglang | `/raid/xrg/sglang`，分支 `flexkv-dsv4-main`（PR #31781，`a2d01bcdb5`） |
| flexkv | `/raid/xrg/FlexKV`，分支 `radixshmem_rebase_v1` |
| shmradix | `/raid/xrg/radixshmem`（editable，component API：`insert(..., *, component=)`） |
...
```bash
uv venv /raid/xrg/sglang/.venv-flexkv --python 3.12
VENV=/raid/xrg/sglang/.venv-flexkv
...
> 那套环境（如另一个 vllm-env）之后要用就得各自重编，或先备份两份 `.so`。
...
VIRTUAL_ENV=$VENV uv pip install -e /raid/xrg/radixshmem/python --no-build-isolation --no-deps

# flexkv editable（若还没）+ 它的运行时依赖（venv 里默认缺）
VIRTUAL_ENV=$VENV uv pip install -e /raid/xrg/FlexKV --no-deps
...
> **MPS 坑**：本机系统默认管道 `/tmp/nvidia-mps` 后面挂着一个对我们不可用的 MPS
> 守护进程，任何 CUDA 程序看到它就会去连，结果 `Error 805` / `cuda.is_available()`
...
export FLEXKV_ENABLE_MPS=0         # 本机必须。默认 1 会执行 nvidia-cuda-mps-control -d，
...
$VENV/bin/python -m sglang.launch_server \
    --model-path /raid/model/DeepSeek-V4-Flash-FP8 \
    --port 30000 \
    --tp-size 4 \
    --enable-flexkv \
    --flexkv-config-file /raid/xrg/flexkv_dsv4.yaml \
    --mem-fraction-static 0.75 \
    --max-running-requests 8 \
    > /tmp/sglang.log 2>&1
```

本机两份权重：`/raid/model/DeepSeek-V4-Flash-FP8`（274 GB）和 `/raid/model/DeepSeek-V4-Flash`
（149 GB，expert 是 fp4）。H20 没有 FP4 硬件，用 FP8 那份；tp=4 时每卡权重 68.8 GB，
`--mem-fraction-static` 至少 0.75（0.45 装不下权重）。权重在 page cache 里时约 25 秒加载完。
...
### 4.4 SWA 正确性验证方案（2026-09-06/07 实测）
...
| 字节 | `tests/test_layerwise_dsv4_multi_group_swa_roundtrip.py`、`tests/test_swa_control_plane_e2e.py` | GPU→CPU→GPU 逐字节相等 | 通过 |
...
BASE, MODEL, NEW = "http://127.0.0.1:30000", "/raid/model/DeepSeek-V4-Flash-FP8", 128
...
|---|---|---|---|
| 正常 | 0.0666 / 0.25 | 0.0749 / 0.47 | 7/24 / 7/24 |
| `ZERO_SWA_ON_FLUSH=1` | 0.0064 / 0.043 | 0.0070 / 0.048 | 7/24 / 7/24 |
| `ZERO_SWA_TAIL=1` | 2.0e5 / 4.5e6 | 0.028 / 0.15 | 0/24 / 7/24 |

host 对 device 与 device 对 device 逐 prompt 配对跟随，逐位相同的是同样 7 条短 prompt；末页清零后
24 条全部改变。结论：**FlexKV 恢复的 SWA 窗口字节正确且被模型消费**。

#### 4.4.5 复现步骤

```bash
# 1. 按 §3.3 启动服务，多加一个环境变量（需要 4.4.3 的钩子在工作区）
SGLANG_DEBUG_ZERO_SWA_ON_FLUSH=1 ... python -m sglang.launch_server ...
# 2. 跑 4.4.2 的脚本（首次需 HTTPS_PROXY 下载 LongBench-v2，之后有本地缓存），约 50 s
# 3. 可选：换 SGLANG_DEBUG_ZERO_SWA_TAIL=1 重启再跑一次，确认 KL 爆炸
```

脚本已放在 `tests/tools/`（gitignore，见其 README）。尚未做的：正式化并接 CI；kv-canary 的 DSv4 适配器补全（长期字节级护栏）；
