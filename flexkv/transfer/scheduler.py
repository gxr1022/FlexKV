...
class TransferScheduler:
    ...
    def fail_graph(self, graph_id: int) -> None:
        ...
        drain; schedule() ignores finished ops whose graph is gone, and
        _drain_bucket drops the stale dirty bit of a removed graph. Idempotent,
        and a no-op for graphs that already completed. Failed prefetch graphs
        live in the background bucket, so both buckets are cleared."""
        self._fg.pop(graph_id, None)
        self._bg.pop(graph_id, None)
